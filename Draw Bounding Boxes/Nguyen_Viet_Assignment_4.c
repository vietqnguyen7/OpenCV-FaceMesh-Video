...
int main (int argc, char *argv[]) 
{
	...
	for(frames = 1; frames <= numFrames; frames++)
	{
		snprintf(vidlocation, sizeof vidlocation, "%s/out%d.png",videoid,frames);

		//Converts to greyscale
		source_image_gray = imread (vidlocation, CV_LOAD_IMAGE_GRAYSCALE); //Converts to greyscale
		...
		imwrite(vidlocation,source_image_gray);
		//DATABASE PARAMETERS
		//Queries video_id
		...
		strcat(dbInput,lastChar);
		db_result = PQexec(db_connection, dbInput);
		if (PQresultStatus(db_result) != PGRES_COMMAND_OK  && frames == 1)
	    	{
		...
		//INPUTS FACE 
		snprintf(dbInput, sizeof dbInput, "INSERT INTO bounding_box_face (video_id,frame,x,y,width,height) VALUES (%s,%d,%d,%d,%d,%d)",videoid,frames,bounding_box_face_x,bounding_box_face_y,bounding_box_face_width,bounding_box_face_height);
		db_result = PQexec(db_connection, dbInput);
		if (PQresultStatus(db_result) != PGRES_COMMAND_OK)
	    	{
		...
		//INPUTS LEFT EYE
		snprintf(dbInput, sizeof dbInput, "INSERT INTO bounding_box_left_eye (video_id,frame,x,y,width,height) VALUES (%s,%d,%d,%d,%d,%d)",videoid,frames,bounding_box_left_eye_x,bounding_box_left_eye_y,bounding_box_left_eye_width,bounding_box_left_eye_height);
		db_result = PQexec(db_connection, dbInput);
		if (PQresultStatus(db_result) != PGRES_COMMAND_OK)
	    	{
		...
		//INPUTS RIGHT EYE
		snprintf(dbInput, sizeof dbInput, "INSERT INTO bounding_box_right_eye (video_id,frame,x,y,width,height) VALUES (%s,%d,%d,%d,%d,%d)",videoid,frames,bounding_box_right_eye_x,bounding_box_right_eye_y,bounding_box_right_eye_width,bounding_box_right_eye_height);
		db_result = PQexec(db_connection, dbInput);
		if (PQresultStatus(db_result) != PGRES_COMMAND_OK)
	    	{
		...
		//INPUT NOSE
		snprintf(dbInput, sizeof dbInput, "INSERT INTO bounding_box_nose (video_id,frame,x,y,width,height) VALUES (%s,%d,%d,%d,%d,%d)",videoid,frames,bounding_box_nose_x,bounding_box_nose_y,bounding_box_nose_width,bounding_box_nose_height);
		db_result = PQexec(db_connection, dbInput);
		if (PQresultStatus(db_result) != PGRES_COMMAND_OK)
	    	{
		...
		//INPUT MOUTH
		snprintf(dbInput, sizeof dbInput, "INSERT INTO bounding_box_mouth (video_id,frame,x,y,width,height) VALUES (%s,%d,%d,%d,%d,%d)",videoid,frames,bounding_box_mouth_x,bounding_box_mouth_y,bounding_box_mouth_width,bounding_box_mouth_height);
		db_result = PQexec(db_connection, dbInput);
		if (PQresultStatus(db_result) != PGRES_COMMAND_OK)
	    	{
